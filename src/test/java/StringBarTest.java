...
public class StringBarTest {
    ...
    public void addObserver() {
        Bar bar = new StringBar();
        HumanClient humanClientMock = Mockito.mock(HumanClient.class);
        bar.addObserver(humanClientMock);
        Mockito.verify(humanClientMock, Mockito.never()).happyHourStarted(bar);
        Mockito.verify(humanClientMock, Mockito.never()).happyHourEnded(bar);
        bar.startHappyHour();
        Mockito.verify(humanClientMock, Mockito.times(1)).happyHourStarted(bar);
        Mockito.verify(humanClientMock, Mockito.never()).happyHourEnded(bar);
        bar.endHappyHour();
        Mockito.verify(humanClientMock, Mockito.times(1)).happyHourStarted(bar);
        Mockito.verify(humanClientMock, Mockito.times(1)).happyHourEnded(bar);
    }
    @Test
